...
function setupBgmToggle() {
  ...
  updateUI();
    ...
    function setupCgcArtifact3D() {
      ...
      const rim = new THREE.PointLight(0x9edfff, 5.5, 10);
          ...
          model.traverse((obj) => {
            ...
            if (obj.material) {
              ...
              materials.forEach((material) => {
                ...
                if ("roughness" in material) material.roughness = Math.min(material.roughness, 0.32);
                if ("metalness" in material) material.metalness = Math.max(material.metalness, 0.5);
              });
            }
          });
      ...
      function animate(now) {
        ...
        if (model && active) {
          ...
          targetY = Math.sin(time * 0.18) * 0.12 + pointerX * 0.15;
          currentY += (targetY - currentY) * 0.035;

          model.rotation.y = currentY;
          model.rotation.x = 0.08 + pointerY * -0.05 + Math.sin(time * 0.23) * 0.025;
          model.position.y = Math.sin(time * 0.42) * 0.04;
          model.position.z = Math.sin(time * 0.21) * 0.025;

          if (hovering) {
            rim.intensity += (6.8 - rim.intensity) * Math.min(dt * 2.5, 1);
          } else {
            rim.intensity += (5.5 - rim.intensity) * Math.min(dt * 2.0, 1);
          }
        }
