// CGC v5: UI hover/click synthetic sound effects intentionally removed.
...
import { OrbitControls } from "https://unpkg.com/three@0.160.0/examples/jsm/controls/OrbitControls.js";
        ...
        // CGC MICRO DUST — cinematic monochrome particles
        (function () {
          ...
          function resetParticle(p, randomPosition = true) {
            p.x = randomPosition ? Math.random() * W : (Math.random() < .5 ? -8 : W + 8);
            p.y = Math.random() * H;
            p.z = Math.random();
            p.r = (0.22 + Math.random() * 0.72) * dpr;
            p.vx = (Math.random() - .5) * 0.055 * dpr;
            p.vy = (-0.018 + (Math.random() - .5) * 0.035) * dpr;
            p.alpha = 0.08 + Math.random() * 0.30;
          ...
          function resize() {
            ...
            const count = Math.min(320, Math.max(150, Math.round(innerWidth * innerHeight / 6200)));
            parts = Array.from({ length: count }, () => { const p = {}; resetParticle(p, true); return p; });
          }
          resize();
          addEventListener("resize", resize, { passive: true });
          addEventListener("pointermove", (e) => { mouse.x=e.clientX*dpr; mouse.y=e.clientY*dpr; mouse.active=true; }, { passive:true });
          ...
          function draw(t) {
            ...
            for (const p of parts) {
              if (mouse.active) {
                ...
                const range=150*dpr;
                if (d2 < range*range && d2 > 16) {
                  const d=Math.sqrt(d2), f=(1-d/range)*0.00018;
                  p.vx += dx*f; p.vy += dy*f;
                }
              }
              p.vx *= .992; p.vy *= .996;
              p.x += p.vx; p.y += p.vy;
              p.twinkle += .008 + p.z*.012;
              if (p.x < -12 || p.x > W+12 || p.y < -12 || p.y > H+12) resetParticle(p, false);
              const pulse=.72+Math.sin(p.twinkle)*.28;
              const a=p.alpha*pulse;
              const radius=p.r*(.65+p.z*.65);
              ctx.beginPath();
              ctx.fillStyle=`rgba(232,242,247,${a})`;
        ...
        // CGC v5: creator chat feature removed.
...
function setupBgmToggle() {
  ...
  updateUI();
    ...
    function setupCgcArtifact3D() {
      ...
      const key = new THREE.DirectionalLight(0xeaf7ff, 3.4);
      key.position.set(3.4, 4.2, 5.0);
      key.intensity = 2.15;
      scene.add(key);

      const rim = new THREE.PointLight(0xe8f8ff, 2.8, 10);
      rim.position.set(-2.6, 1.8, 3.0);
      scene.add(rim);

      const scanLight = new THREE.PointLight(0xffffff, 0, 5.5);
          ...
          model.traverse((obj) => {
            ...
            if (obj.material) {
              ...
              materials.forEach((material) => {
                if (material.color) material.color.setRGB(0.12, 0.14, 0.15);
                if ("roughness" in material) material.roughness = 0.18;
                if ("metalness" in material) material.metalness = 0.94;
                if ("emissive" in material) { material.emissive.setRGB(0.008, 0.012, 0.014); material.emissiveIntensity = 0.16; }
      ...
      function animate(now) {
        ...
        if (model && active) {
          ...
          scanLight.intensity = 0.35 + Math.pow(Math.sin(time * 0.72), 8) * 5.2;
          if (hovering) {
            rim.intensity += (4.8 - rim.intensity) * Math.min(dt * 3.0, 1);
            scanLight.intensity *= 1.55;
          } else {
            rim.intensity += (2.8 - rim.intensity) * Math.min(dt * 2.0, 1);
          }
        }
    ...
    // 初期化
       ...
       if (heroVideo) {
         const toggleKey = "cgc_hero_video_toggle_v5";
         const previous = localStorage.getItem(toggleKey) || "2";
         const next = previous === "1" ? "2" : "1";
         localStorage.setItem(toggleKey, next);
         heroVideo.src = next === "1" ? "media/cgc-hero.mp4" : "media/cgc-hero-2.mp4";
         heroVideo.load();
